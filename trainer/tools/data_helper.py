...
class DataHelper:
    ...
    def prepare_data_aug(self, data_dict, want_reverse=False):
            ...
            inputs_argue = [data_dict[k] for k in input_keys]
            batch_size = len(inputs_argue[0])
            targets_argue = [data_dict[k] for k in target_keys]

            sequences = [
                self._prepare_sequence(inputs_argue, force_list=False),
                self._prepare_sequence(targets_argue, force_list=False)
            ]
            if want_reverse:
                rev_data_dict = self._reverse_data_dict(data_dict)
                sequences.extend([
