...
from ProtoSeg.lib_PMSAD.utils.tools.configer import Configer
...
if __name__ == "__main__":
    ...
    from ProtoSeg.lib_PMSAD.utils.distributed import handle_distributed
    ...
    with open('args_values.txt', 'w') as f:
        for key, value in configer.get().items():
            f.write(f"{key}: {value}\n")
    print("para_saved")
    ...
    if configer.get('method') == 'fcn_segmentor':
        if configer.get('phase') == 'train':
            ...
            from trainer.tester_offset import Tester
            model = Tester(configer)
    else:
